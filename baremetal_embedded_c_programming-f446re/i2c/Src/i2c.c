...
void i2c1_dummy_start(void)
{
    ...
    /* Read SR1 then write DR (required sequence) */
    temp = I2C1->SR1;

    /* Send dummy address: 0x50 (write) */
    I2C1->DR = (0x50 << 1);
...
void i2c1_scan() {
	...
	// for each address from 0x08 to 0x77

	// send start signal

	// wait for ACK

	// if no ACK, go to next address

	// if ACK, return address found
}
